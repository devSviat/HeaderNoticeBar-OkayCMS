...
```bash
git clone https://github.com/devSviat/HeaderNoticeBar-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/HeaderNoticeBar/     ← модуль
└── tests/Modules/Sviat/HeaderNoticeBar/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/HeaderNoticeBar
cp -r ~/HeaderNoticeBar-OkayCMS/tests/. tests/Modules/Sviat/HeaderNoticeBar/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/HeaderNoticeBar
```
...
2. Неймспейс — `Modules\Sviat\HeaderNoticeBar`.
...
use Okay\Modules\Sviat\HeaderNoticeBar\Services\SomeService;
